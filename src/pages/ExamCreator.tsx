...
export default function ExamCreator({ currentUser, subjects, onBack, onNavigate, onReload }: ExamCreatorProps) {
    ...
    <div className="flex-1 flex flex-col h-screen overflow-hidden bg-[var(--bg-body)] text-white relative">
      <div className="flex items-center justify-between p-6 lg:px-12 border-b border-white/5 bg-black/20 backdrop-blur-md z-10 shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black font-display tracking-tight">Criador de Provas</h1>
            <p className="text-xs text-zinc-400 font-medium">Crie exames oficiais com temporizador e janela síncrona</p>
          </div>
        </div>
        ...
        <div className="max-w-5xl mx-auto space-y-8 pb-32">
          ...
          <div className="glass border border-white/5 rounded-[32px] p-8 md:p-10 space-y-8">
            <div className="flex items-center gap-3 border-b border-white/5 pb-4">
              ...
              <h2 className="text-xl font-bold font-display">Configurações Gerais da Prova</h2>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2 md:col-span-2">
                <label className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Título da Prova</label>
                <input 
                  type="text" 
                  value={title}
              ...
              <div className="space-y-2">
                ...
                <input 
                  ...
                  placeholder="Ex: 2.0"
                  className="w-full bg-black/40 border border-white/10 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-orange-500 transition-colors font-bold"
                />
              </div>
              ...
              {autoDuration && (
                <motion.div
                  ...
                  className="md:col-span-2 flex items-center gap-3 bg-orange-500/10 border border-orange-500/20 rounded-2xl px-5 py-4"
                >
                  <div className="w-9 h-9 rounded-xl bg-orange-500/20 flex items-center justify-center shrink-0">
                    <Clock size={18} className="text-orange-400" />
                  </div>
                  <div>
                    <p className="text-[10px] font-extrabold text-orange-400 uppercase tracking-widest">Duração da Prova (calculada automaticamente)</p>
                    <p className="text-lg font-black text-white">{autoDuration.label}</p>
                  </div>
                  <div className="ml-auto flex items-center gap-1 text-orange-400/70">
                    <Info size={14} />
              ...
              {startDate && deadlineDate && !autoDuration && (
                <motion.div
                  initial={{ opacity: 0, y: -8 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="md:col-span-2 flex items-center gap-3 bg-red-500/10 border border-red-500/20 rounded-2xl px-5 py-4"
                >
                  <div className="w-9 h-9 rounded-xl bg-red-500/20 flex items-center justify-center shrink-0">
                    <Info size={18} className="text-red-400" />
                  </div>
                  <p className="text-sm font-bold text-red-400">O início deve ser anterior ao fim da janela.</p>
                </motion.div>
              )}

              {/* Seleção de Imagem Capa */}
              <div className="space-y-4 md:col-span-2 border-t border-white/5 pt-6">
                <label className="text-xs font-bold text-zinc-400 uppercase tracking-widest block">Imagem de Capa da Prova</label>
                    ...
                    return (
                      <div 
                        ...
                        className={`h-24 rounded-2xl overflow-hidden relative cursor-pointer border-2 transition-all ${
                          isSelected ? 'border-orange-500 scale-[1.03] shadow-lg shadow-orange-500/20' : 'border-white/10 opacity-70 hover:opacity-100'
                        }`}
                      >
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Ou insira uma URL de imagem customizada</label>
                  <div className="relative">
                    ...
                    <input 
                      ...
                      className="w-full bg-black/40 border border-white/10 rounded-2xl pl-14 pr-5 py-4 text-sm text-white focus:outline-none focus:border-orange-500 transition-colors font-medium"
                    />
                  </div>
                </div>
              </div>
            ...
            <AnimatePresence>
              {questions.map((q, index) => (
                <motion.div 
                  ...
                  className="glass border border border-white/10 rounded-[32px] p-8 relative group"
                >
                  <button 
