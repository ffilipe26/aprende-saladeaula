  ...
  return (
    ...
    >
      ...
      <motion.div 
        ...
        className="glass border border-orange-500/10 rounded-[32px] p-8 md:p-10 relative overflow-hidden flex flex-col xl:flex-row items-center justify-between gap-8 bg-gradient-to-br from-orange-600/5 to-transparent"
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full xl:w-auto shrink-0 z-10">
          ...
          <motion.button 
            ...
            className="bg-white/5 hover:bg-white/10 border border-white/5 text-white p-5 rounded-2xl font-extrabold flex flex-col items-center justify-center gap-2.5 transition-all text-center min-w-[130px]"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {teacherKpis.map((kpi, idx) => (
          <motion.div 
            key={idx} 
            variants={itemVariants}
            className="glass border border-[var(--border)] rounded-3xl p-6 group hover:border-orange-500/20 transition-all duration-300"
          >
            <div className="flex justify-between items-start mb-6">
              <span className="text-[10px] font-extrabold text-[var(--text-muted)] uppercase tracking-wider">{kpi.label}</span>
              <div className={`p-3 rounded-2xl bg-zinc-800/80 ${kpi.color} ring-1 ring-white/5 shadow-inner`}>
                <kpi.icon size={20} />
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <motion.div 
          variants={itemVariants}
          className="lg:col-span-2 glass border border-[var(--border)] rounded-[32px] p-8 space-y-6"
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {subjects.map((sub) => (
              <div 
                key={sub.id} 
                className="p-6 bg-white/5 border border-white/5 rounded-2xl hover:border-orange-500/20 transition-all group flex flex-col justify-between"
              >
                <div>
                ...
                <div className="flex justify-between items-center pt-4 border-t border-white/5 text-[10px] font-extrabold uppercase tracking-widest text-[var(--text-muted)]">
        ...
        <motion.div 
          variants={itemVariants}
          className="glass border border-[var(--border)] rounded-[32px] p-8 space-y-6 flex flex-col justify-between"
        >
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              {pendingSubmissions.map((sub) => (
                <div key={sub.id} className="p-4 bg-white/5 border border-white/5 rounded-2xl flex flex-col gap-3 group hover:border-orange-500/20 transition-all">
          ...
          <button 
            ...
            className="w-full py-4 text-center text-xs font-extrabold uppercase tracking-widest text-[var(--text-muted)] hover:text-white hover:bg-white/5 rounded-2xl transition-all border border-dashed border-[var(--border)] mt-4"
          >
