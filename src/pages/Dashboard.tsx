import { Clock, Play, ChevronRight, GraduationCap, Calendar, ArrowUpRight, Target, Zap, PlusCircle, ClipboardList } from 'lucide-react';
import { motion } from 'motion/react';
import { Activity, Notification, UserRole } from '../types';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        >
          <div className="relative z-10 max-w-lg">
            ...
            <h2 className="text-3xl md:text-5xl font-extrabold mb-6 font-display tracking-tight leading-tight">Foque nas suas Prioridades</h2>
            <p className="text-white/70 mb-10 leading-relaxed text-base md:text-lg font-medium">Você tem {pendingActivities.length} atividades pendentes para entregar. Organize seu tempo e não perca os prazos das suas disciplinas.</p>
            {pendingActivities.length > 0 ? (
              <button 
                onClick={() => onStartActivity(pendingActivities[0].id)}
                className="bg-white text-orange-600 px-8 py-4 rounded-2xl font-extrabold flex items-center gap-3 hover:bg-orange-50 hover:scale-105 transition-all shadow-xl active:scale-95"
              >
                <Play size={22} fill="currentColor" />
                Começar {pendingActivities[0].title}
              </button>
            ) : (
              <button className="bg-white/20 text-white px-8 py-4 rounded-2xl font-extrabold flex items-center gap-3 cursor-default">
        ...
        {/* ==========================================
            ÁREA DO PROFESSOR (Atalhos de Criação)
            ========================================== */}
        {role === 'teacher' && (
