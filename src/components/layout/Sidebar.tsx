...
export default function Sidebar({ 
  ...
  const menuItems = allMenuItems.filter((item) => {
    // Alunos não têm acesso a Membros (gerenciamento)
    if (userRole === 'student' && item.id === 'members') return false;
    // Administradores não precisam de telas operacionais pedagógicas (Aulas, Atividades, Provas)
    // Essas são responsabilidades de Professores e Alunos — o Admin gerencia a instituição
    if ((userRole === 'admin' || userRole === 'super_admin') && ['lessons', 'activities', 'exams'].includes(item.id)) return false;
    return true;
  });
