...
import { Activity, Question, QuestionType, Subject } from '../../types';

interface ActivityFormProps {
  activity?: Activity;
  onSave: (activity: Activity) => void;
  onCancel: () => void;
  subjects?: Subject[]; // Disciplinas disponíveis para vinculação relacional
}

export default function ActivityForm({ activity, onSave, onCancel, subjects = [] }: ActivityFormProps) {
  // ==========================================
  // ESTADOS DO FORMULÁRIO
  // ==========================================
  const [title, setTitle] = useState(activity?.title || '');
  const [subjectId, setSubjectId] = useState(activity?.subjectId || (subjects.length > 0 ? subjects[0].id : ''));
  const [subject, setSubject] = useState(activity?.subject || (subjects.length > 0 ? subjects[0].name : ''));
  ...
  const handleSave = () => {
    ...
    const newActivity: Activity = {
      ...
      title,
      subject,
      subjectId, // Vínculo relacional com a disciplina selecionada
      teacherId: '', // placeholder, should be passed from App
      deadlineDate,
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="space-y-3">
                  <label className="text-[10px] font-extrabold text-[var(--text-muted)] uppercase tracking-[0.2em] ml-1">Disciplina / Turma</label>
                  <div className="relative">
                    <select 
                      ...
                      onChange={(e) => {
                        ...
                        setSubject(selected?.name || e.target.value);
                      }}
